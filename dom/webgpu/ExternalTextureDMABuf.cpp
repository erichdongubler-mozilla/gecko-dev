...
void ExternalTextureDMABuf::SetSubmissionIndex(uint64_t aSubmissionIndex) {
  ExternalTexture::SetSubmissionIndex(aSubmissionIndex);
}
...
void ExternalTextureDMABuf::onBeforeQueueSubmit(RawId aQueueId) {
  ...
  mVkSemaphoreHandle = nullptr;
  const ffi::WGPUVkSemaphoreHandle* vkSemaphore =
